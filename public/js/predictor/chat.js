...
const SUGGESTED_PROMPTS = [
    { icon: '🌡', text: '¿Cómo están las zonas ahora?' },
    { icon: '🚨', text: '¿Hay algún sensor en alerta?' },
    { icon: '📈', text: 'Gráfica de temperatura de Engorda últimas 6 horas' },
    { icon: '💧', text: '¿Cómo está la humedad en Habita 1?' },
    { icon: '📊', text: 'Genera un reporte PDF de la última semana' },
    { icon: '🎯', text: '¿Cuáles son los rangos óptimos?' },
];
...
function buildEmptyHTML() {
    ...
    return `
        <div class="ai-chat-empty">
            <strong>👋 ¡Hola!</strong> Soy el Asistente de Malinalco.<br>
...
export function initChat() {
    ...
    // Click en un chip de sugerencia: inyecta la pregunta y la envia.
    ...
    // Atajo C para abrir/cerrar (salvo que el usuario este tipeando en un input).
...
function renderChartInto(parentEl, spec) {
    ...
    const textColor = '#E0E5E8';
    const axisColor = '#5b7888';
...
// Markdown lite a mano: negritas **x**, codigo `x`, listas (- x), tablas pipe.
...
async function onSubmit(e) {
    ...
    if (isReportRequest) {
        ...
        const steps = [
            ...
            { at: 10, text: 'Calculando estadísticas…' },
            { at: 18, text: 'Generando gráficas…' },
            { at: 26, text: 'Consultando el LLM para redactar el resumen…' },
            { at: 46, text: 'Redactando recomendaciones…' },
            { at: 62, text: 'Renderizando PDF…' },
        ...
        progressTimer = setInterval(() => {
            ...
            if (msgEl.textContent !== current.text) msgEl.textContent = current.text;
        }, 1000);
    }
